...
const settings = {
  map: {
    maxX: 20, //клеток по горизонтали
    maxY: 12, // клеток по вертикали
    cellSize: 49, // размер клетки
    minRooms: 5, // нижняя граница количества комнат
    maxRooms: 10, // верхняя граница количества комнат
    minRoomSize: 3, // нижняя граница размера комнаты
    maxRoomSize: 8, // верхняя граница размера комнаты
    minCorridors: 3, // нижняя граница количества коридоров
    maxCorridors: 5, // верхняя граница количества коридоров
    corridorWidth: 1, // ширина коридора
  },

  gameElements: {
    HP: {
      quantity: 10, // количество зелий
      effectValue: 30, // размер хилки
      type: 'HP', // флаг зелья
    },
    swords: {
      quantity: 2, // количество мечей
      effectValue: 10, // усиление от меча
      actionTime: 3000, // время действия усиления
      type: 'SW', // флаг меча
    }
  },
  ...
  enemies: {
    type: 'E', // флаг врага
    damage: 10, // урон врага
    attackSpeed: 1000, // скорость атаки (период в мс)
    speed: 800, // скорость врагов (период в мс)
    quantity: 10, // количество врагов

  },

  protagonist: {
    attackPower: 20, // сила удара героя
    type: 'P', // флаг героя
  }
...
class Game {
  ...
  gameOver(isLoss) {
    if (!this.isOver) {
      ...
      $('.field').empty();
      this.init(isLoss ? 'Увы! Ты проиграл! ' : 'Ура! Победа! ');
    }
  }

  init(message = '') {
    $('.field').append(`<div id="hover" class="field__hover"><b>${message}<b><br/><br/>Нажмите любую кнопку, чтобы начать новую игру</div></div>`);
